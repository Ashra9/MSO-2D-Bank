...
loan_default_update <- function() {
  ...
  for (i in 1:nrow(loanData)) {
    risk <- loanData$loan_risk[i]
    if (runif(1) < risk) {
      rows_to_remove <- c(rows_to_remove, i)
    }
  }
  if (length(rows_to_remove) > 0) {
    updatedLoanData <- loanData[-rows_to_remove, ]
  } else {
    updatedLoanData <- loanData
  }
  return(updatedLoanData)
}

# Call the function to update the loan values with their respective risk of default
updatedLoanData <- loan_default_update()
print(updatedLoanData)
