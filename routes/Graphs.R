...
plotCashGraph <- function(input, output, vals){
  ...
  output$graphs <- renderUI({
    ...
    if (is.null(vals$cashInventory)) {
      ...
      # # loan liquidation plot
      # loanLiquidation <- vals$completedLoansLiquidated
      # print("Loan Liquidation")
      # print(loanLiquidation)
      # if (nrow(subset(loanLiquidation, month == vals$current_month))==0) {
      #   loanLiquidation <- rbind(loanLiquidation, data.frame(month=c(vals$current_month),loanID=c(0), loanValue=c(0)))
      # }
      # 
      # cum_loanLiquidation <- loanLiquidation %>% group_by(month) %>% 
      #   summarise(sum_loanValue = sum(loanValue),
      #             .groups = 'drop') %>% mutate(cum_loanLiquidation = cumsum(sum_loanValue))
      # print(cum_loanLiquidation)
      # 
      # df_cum_value_loan_liquidation <- bind_rows(old = cum_loanLiquidation, 
      #                                           new = cum_loanLiquidation %>% mutate(cum_loanLiquidation = lag(cum_loanLiquidation)),
      #                                           .id = "source") %>%
      #   arrange(month, source)
      # print(df_cum_value_loan_liquidation)
      # 
      # # loan default plot
      # loanDefault <- vals$completedLoansDefaulted
      # if (nrow(subset(loanDefault, month == vals$current_month))==0) {
      #   loanDefault <- rbind(loanDefault, data.frame(month=c(vals$current_month),loanID=c(0), loanValue=c(0)))
      # }
      # 
      # cum_loanDefault <- loanDefault %>% group_by(month) %>% 
      #   summarise(sum_loanValue = sum(loanValue),
      #             .groups = 'drop') %>% mutate(cum_loanDefault = cumsum(sum_loanValue))
      # 
      # df_cum_value_loan_default <- bind_rows(old = cum_loanDefault, 
      #                                            new = cum_loanDefault %>% mutate(cum_loanDefault = lag(cum_loanDefault)),
      #                                            .id = "source") %>%
      #   arrange(month, source)
